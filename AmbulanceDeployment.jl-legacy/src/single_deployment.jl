...
## make sure to include @time begin
    ...
    for(deployment_model, name) in ((dp -> RobustDeployment(dp, α=0.0000001), :RobustCertain),
                                      (dp -> RobustDeployment(dp, α=0.5), :RobustMiddle),
                                      (dp -> RobustDeployment(dp, α=0.01), :RobustDesired),
                                      (dp -> RobustDeployment(dp, α=0.99999), :RobustUncertain))
        println("$name: ")
    ...
    JLD.jldopen("outputs/austin_robust_test.jld", "w") do file
    ...
    solverstats = JLD.load("outputs/austin_robust_test.jld")
    amb_deployment = solverstats["amb_deployment"]
    json_string = JSON.json(amb_deployment)
    open("../src/outputs/austin_robust_test.json","w") do f
        write(f, json_string)
    end
